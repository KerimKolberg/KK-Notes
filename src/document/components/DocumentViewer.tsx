 ...
 */
export const DocumentViewer = memo(function DocumentViewer({ settingsRef, currentTool }: DocumentViewerProps) {
  ...
  // `[singlePage]` must not be built inline: a new array each render would
  // bust the layout memo below on every unrelated state change, and laying
  // out the strip again is the most expensive thing this component does.
  const layoutInput = useMemo(() => (singlePage ? [singlePage] : pages), [singlePage, pages]);
